...
from feed_reader import app, db, bcrypt, methods
...
def home():
    ...
    if  url_rss != None:
        feed = methods.validate_url(url_rss)
        if  feed == True:
            ...
            if (source_rss['title'] in source_list):
                ...
                source = Source(title=source_rss['title'], subtitle=source_rss['subtitle'], 
                                link=source_rss['link'], user_id=current_user.id, feed=url_rss)
                ...
                for article in articles_rss:
                    ...
                    articles = Article(title=title, body=body, link=link, guid=guid, 
                                        date_published=published ,source_id=source.id, user_id=current_user.id)
            ...
            while parsed.status != 200 or n>3:
                ...
                if articles_up[0]['title'] in articles_list:
                    ...
                    for article in articles_up:
                        ...
                        articles = Article(title=title, body=body, link=link, guid=guid, date_published=published, 
